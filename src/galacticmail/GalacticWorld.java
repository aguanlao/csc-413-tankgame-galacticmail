        ...
        if (!GameOver) {
            ...
            //for (int i = 0; i < objects.size(); i++) {
                ...
                if (obj instanceof Planet) {
                    ...
                    // check if player is already landed and is flying off
                    if (isNear(base, player) && base.isLandedOn() && player.leftThePlanet()) {
                        objIterator.remove();
                    }
                    
                    else if (isNear(base, player) && player.collides(base)) {
                        player.setLanding(obj);
                        ((Planet)obj).setIsLandedOn(true);
